...
def getNet(act_dim):
    ...
    devs = mx.gpu(0)
    #devs = mx.cpu()
    loss_net = sym.get_symbol_test_bn(act_dim)
    loss_mod = A3CModule(loss_net, data_names=('data',),
    ...
    loss_mod.init_optimizer(optimizer='adam',
                            optimizer_params={'learning_rate': args.lr, 'wd': args.wd, 'epsilon': 1e-3, 'clip_gradient': 40.0})
    return loss_mod
...
def actor_learner_thread(thread_id):
    ...
    while T < TMAX:
        ...
        while not (terminal or ((t - t_start) == args.t_max)):
            ...
            _, value_out, _, policy_out = module.get_outputs()
        ...
        neg_adv_v = -np.array(td_minibatch).reshape((-1,))
        neg_adv = np.zeros((len(neg_adv_v), act_dim), dtype=np.float32)
        #print neg_adv.shape, np.array(list(a_minibatch)).shape
        neg_adv[np.arange(neg_adv_v.shape[0]), a_minibatch] = neg_adv_v
        v_grad = mx.nd.array(np.array(neg_adv_v * 0.5).reshape((-1,1)))
                    
        batch = mx.io.DataBatch(data=[mx.nd.array(s_minibatch)],label=None)
        with lock:
